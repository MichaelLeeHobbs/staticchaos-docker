...
**Primary sources:** `src/fist.c`, `src/fight.c`, `src/update.c`, `src/handler.c`, `src/const.c`, `src/merc.h`, `src/interp.c`
...
- **Damage types** route through `damage()` in `src/fight.c`; the melee `F_*` types
...
Source: `src/const.c` `class_table[]` (the `"Fist"` row), `src/merc.h` (struct `class_type`).
...
`NEW_PHOENIX_AURA`=262144 (offense aura). **Melee damage types** (`src/merc.h`):
...
All Fist commands are `level 2` in the command table (`src/interp.c`); position is noted per ability.
...
*Source: `src/fist.c:do_master` · `interp.c`: POS_STANDING, level 2*
...
*Source: `src/fist.c:do_discipline` · POS_STANDING, level 2*
...
*Source: `src/fist.c:do_bodytrain` · `interp.c`: POS_FIGHTING, level 2*
...
*Source: `src/fist.c:gain_ki` (helper, not a command)*
...
- **Decay:** out of combat, a tick with `F_KI > 0` and no `NEW_PHOENIX_AURA` collapses Ki to 0
  ("The aura of Ki around you collapses." — `src/update.c`, the Fist regen branch).

---

## 3. Combo — `combo`
*Source: `src/fist.c:do_combo` · `interp.c`: POS_FIGHTING, level 2*
...
These blazes proc **bonus damage on every normal punch (`TYPE_HIT`) vs an NPC** (`src/fight.c:damage`,
...
### Eyesight — `eyesight`
*Source: `src/fist.c:do_eyesight` · POS_STANDING*

- **Gate:** `F_DISC >= 1`.
- **Effect:** toggles `PLR_TRUESIGHT` (free, no cost, no lag).

### Levitate — `levitate`
*Source: `src/fist.c:do_levitate` · POS_STANDING*
...
*Source: `src/fist.c:do_roundhouse` · POS_FIGHTING*
...
### Innerfire — `innerfire`
*Source: `src/fist.c:do_innerfire` · POS_FIGHTING*
...
- **Resolve (next tick, `src/update.c` Fist branch):** clears `NEW_INNERFIRE` and **removes every
  affect on the character** (`while (ch->affected) affect_remove(...)`) — strips debuffs *and* buffs.

### Phoenixaura — `phoenixaura`
*Source: `src/fist.c:do_phoenixaura` (toggle) + `src/update.c` (upkeep) · POS_FIGHTING*
...
- **Upkeep (`src/update.c`, Fist branch):** each tick, if `move < 250` the aura **collapses**
  (clears bit, `hitroll -= 15`, `damroll -= 15`); otherwise `move -= 150 + dice(10,10)`.
- **Also collapses** when not `POS_FIGHTING` (`src/update.c` ~line 649: clears bit, removes the
...
*Source: `src/fist.c:do_dim_mak` · `interp.c`: POS_FIGHTING, level 2*
...
## 5. Passive defenses & damage scaling (always-on, `src/fight.c`)
...
- **EVAL rating** (`src/handler.c:eval`, caps how much hitroll/damroll counts in combat):
  `point += F_DISC/2 + Σ_{i=3..6} (powers[i]+1)/20 + max_move/1500 + max_mana/3000`.

### Defense (Fist as victim, `src/fight.c:damage`, `level >= 2`)
...
- **Figure-Eight** (`NEW_FIGUREEIGHT` set): `dam /= 3` **before** the torso reduction — a large blanket
  ...
  decays each tick with probability `100 - body/2` percent (`src/update.c`), so high body = it lasts longer.

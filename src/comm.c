...
#if	defined(linux)
/* These hand-written prototypes predate ANSI headers and now conflict with
 * modern glibc (read and write return ssize_t and take pointer/size_t args;
 * gettimeofday takes a void second arg). The correct declarations already come
 * from unistd.h, sys/time.h, and sys/socket.h, so we drop these legacy ones. */
//int	accept		args( ( int s, struct sockaddr *addr, int *addrlen ) );
//int	bind		args( ( int s, struct sockaddr *name, int namelen ) );

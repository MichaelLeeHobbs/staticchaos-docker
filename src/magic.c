...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	if ( !IS_NPC(ch) )
	{
	    if ( !IS_NPC(victim) && ch != victim && victim->level < 2)
	    {
		send_to_char( "You can't do that on a mortal.\n\r", ch );
		return;
	    }
	...
	ch->mana -= mana;
	(*skill_table[sn].spell_fun) ( sn, lev, ch, vo );
    }
...
void spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( IS_AFFECTED(victim, AFF_BLIND) || number_percent() > level*5/4 ) 
	return;
    ...
    af.location  = APPLY_HITROLL;
    af.modifier  = -4;
    af.duration  = (victim->level < 96 ? level*3 + dice(1,4) : 3);
...
{ AFFECT_DATA *paf;
  ...
  act( "A silvery ray lances toward $N.", ch, NULL, victim, TO_CHAR );
  act( "A silvery ray lances toward you!", ch, NULL, victim, TO_VICT );
  act( "A silvery ray lances toward $N", ch, NULL, victim, TO_NOTVICT );
...
void spell_enchant_weapon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Ok.\n\r", ch );
    sprintf( log_buf, "Enchant used by %s on %s (%d).", ch->name, obj->short_descr, obj->pIndexData->vnum );
    log_string( log_buf );
    return;
}
...
{ CHAR_DATA *victim;
  ...
  if ( target_name[0] == '\0' )
  { send_to_char( "Create a portal to whom?\n\r", ch );
    return;
  }
  if ( ( victim = get_char_world( ch, target_name) ) == NULL )
  { send_to_char( "Your magic fails to find a target.\n\r", ch );
    return;
  }
  if ( !IS_NPC(victim) || victim->level >= 35 )
  { send_to_char( "You failed.\n\r", ch );
    return;
  }
  if ( !victim->in_room || victim->in_room == NULL )
  { send_to_char( "A portal begins to open, then is sucked into oblivion.\n\r", ch );
    return;
  }
  ...
  act( "You create a portal to $N.", ch, NULL, victim, TO_CHAR );
  act( "A shimmering portal appears.\n\r", ch, NULL, victim, TO_ROOM );
  return;
}
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
    ||   IS_SET(victim->in_room->room_flags, ROOM_HQ)
    || ( !IS_NPC(ch) && victim->fighting != NULL )
    || ( !IS_NPC(victim) && victim->pcdata->extras[TIMER] > 0 )
    || ( victim != ch
	...
	&&   (pRoomIndex->vnum < 8100 || pRoomIndex->vnum > 8299) )
	    break;
    }
...
void spell_ventriloquate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_name( speaker, "Alathon" ) )
    {
      act( "Alathon says '`mI don't like people putting words in my mouth.`n'", ch, NULL, NULL, TO_CHAR );
      act( "Alathon says '`mI don't like people putting words in my mouth.`n'", ch, NULL, NULL, TO_ROOM );
      ch->hit = -10;
      ch->mana = 0;
      ch->move = 0;
      update_pos( ch );

...
/* lets a mobile inflict damage on a target */
void do_mpdamage( CHAR_DATA *ch, char *argument )
{
    ...
    {
        bug( "MpDamage - Missing argument(s) from vnum %d.",
                ch->pIndexData->vnum );
        return;
    }
    ...
    {
        bug( "Mpdamage - Victim not in room from vnum %d.",
            ch->pIndexData->vnum );
        return;
    }
    ...
    {
      bug( "MpDamage - Non-numeric second argument from vnum %d.",
      		ch->pIndexData->vnum );
      return;
    }
    dam = number_range( atoi(arg2), atoi(arg3) );

    if ( dam < 1 || dam > 30000 )
    {
      bug( "MpDamage - Damage out of range from vnum %d.",
      		ch->pIndexData->vnum );
      return;
    }
    ...
    if ( IS_AFFECTED( ch, AFF_CHARM ) && ch->master == victim )
    {
        bug( "MpDamage - Charmed mob attacking master from vnum %d.",
            ch->pIndexData->vnum );
        return;
    }

    damage( ch, victim, dam, atoi(arg4) );
    return;
}
...
/* Allows a mob to pick a new target, with strategic reasoning */
void do_mpretarget( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *vch;
  CHAR_DATA *vch_next;
  CHAR_DATA *old;
  CHAR_DATA *victim;
  int priority = 0, previous = 0;
  ...
  for ( vch = ch->in_room->people; vch != NULL; vch = vch_next )
  {
    ...
    switch ( vch->class )
    {
      ...
      case CLASS_MAZOKU:
        ...
        break;
      case CLASS_SORCERER:
        if ( vch->pcdata->powers[SORC_SPEC] == SCHOOL_WHITE )
          priority = 7;
        else if ( vch->pcdata->powers[SORC_SPEC] == SCHOOL_ASTRAL )
          priority = 5;
        else
          priority = 4;
        break;
  ...
  if ( old != victim )
  {
    act( "You says '`mGoodbye, $N.`n'", ch, NULL, victim, TO_CHAR );
    act( "$n says '`mGoodbye, $N.`n'", ch, NULL, victim, TO_ROOM );
    ch->fighting = victim;
  }
...
/* lets a mobile inflict damage on all visible PC targets in it's targets room */
void do_mpareadam( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    char arg3[MAX_INPUT_LENGTH];
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' || arg4[0] == '\0' )
    {
        bug( "MpAreaDam - Missing argument(s) from vnum %d.",
                ch->pIndexData->vnum );
        return;
    }
    ...
    if ( ( victim = get_char_room( ch, arg1 ) ) == NULL )
    {
        bug( "MpAreaDam - Victim not in room from vnum %d.",
            ch->pIndexData->vnum );
        return;
    }
    if ( !is_number( arg2 )  || !is_number( arg3 ) || !is_number( arg4 ) )
    {
      bug( "MpAreaDam - Non-numeric second argument from vnum %d.",
                ch->pIndexData->vnum );
      return;
    }
    dam = number_range( atoi(arg2), atoi(arg3) );
    ...
    if ( dam < 1 || dam > 30000 )
    {
      bug( "MpAreaDam - Damage out of range from vnum %d.",
                ch->pIndexData->vnum );
      return;
    }
    ...
    if ( IS_AFFECTED( ch, AFF_CHARM ) && ch->master == victim )
    {
        bug( "MpAreaDam - Charmed mob attacking master from vnum %d.",
            ch->pIndexData->vnum );
        return;
    }
    ...
    for ( vch = victim->in_room->people; vch != NULL; vch = vch_next )
    {
      ...
      if ( !can_see(ch, vch) )
        continue;
      damage( ch, vch, dam, atoi(arg4) );
    }
    return;
}

/* trans everybody in the room to a given vnum */
void do_mptransall( CHAR_DATA *ch, char *argument )
{
    char             arg1[ MAX_INPUT_LENGTH ];
    ROOM_INDEX_DATA *location;
    CHAR_DATA       *vch;
    ...
    if ( arg1[0] == '\0' )
    {
	bug( "Mptransallr - Bad syntax from vnum %d.", ch->pIndexData->vnum );
	return;
    }

    if ( ( location = find_location( ch, arg1 ) ) == NULL )
    {
      bug( "Mptransall - No such location from vnum %d.",
        ch->pIndexData->vnum );
      return;
    }

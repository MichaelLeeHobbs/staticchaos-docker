...
bool redit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Name:       [%s]\n\rClan:       [%5d]\n\rArea:       [%5d] %s\n\r",
	    pRoom->name, pRoom->clan, pRoom->area->vnum, pRoom->area->name );
    strcat( buf1, buf );
...
bool redit_cnum( CHAR_DATA *ch, char *argument )
{
  ROOM_INDEX_DATA *pRoom;
  int clan;
...
bool medit_clan( CHAR_DATA *ch, char *argument )
{
    MOB_INDEX_DATA *pMob;
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  clan [0-6]\n\r", ch );
	return FALSE;
    }

    if ( atoi(argument) < 0 || atoi(argument) > 6 )
    {
      stc( "Pick a clan between 0 and 6, cunt.\n\r", ch );
      return FALSE;
    }

    pMob->clan = atoi( argument );

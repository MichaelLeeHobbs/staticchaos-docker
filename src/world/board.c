...
{ "General",  	"General discussion",            0,     2,     "all", DEF_INCLUDE,90, NULL, FALSE },
{ "Ideas",	  	"Suggestion for improvement",	 0,     2,     "all", DEF_NORMAL, 120, NULL, FALSE }, 
{ "Announce", 	"Announcements from Immortals",  0,     L_IMM, "all", DEF_NORMAL, 180, NULL, FALSE },
{ "Bugs",	  	"Typos, bugs, errors",		     0,     1,     "imm", DEF_NORMAL, 120, NULL, FALSE },
{ "Personal", 	"Personal messages",			 0,     1,     "all", DEF_EXCLUDE,120, NULL, FALSE }
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	if ( IS_SET(ch->act,PLR_NO_NOTE) )
	{
	  stc( "You are prohibited from posting notes.\n\r", ch );
	  return;
	}
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
	...
	if ( !is_number(arg) )
	{
	  ...
	  {
	    sprintf( buf, "%d notes from %s removed.\n\r", count, arg );
	    stc( buf, ch );
	  }
...
static void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	{
		argument = one_argument(argument,sender);
	  fOnly = TRUE;
	}
	...
	for (p = ch->pcdata->board->note_first; p; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			...
			if( (!fOnly && (!show || ((count-show) < has_shown)))
			 || (fOnly && is_name(sender,p->sender)) )
			{
